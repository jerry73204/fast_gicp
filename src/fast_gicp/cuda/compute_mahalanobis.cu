...
struct compute_mahalanobis_kernel {
  ...
  __host__ __device__ Eigen::Matrix3f operator()(const thrust::tuple<Eigen::Vector3f, Eigen::Matrix3f, int>& input) const {
...
void compute_mahalanobis(
  ...
  CudaExecutionContext ctx("compute_mahalanobis");
